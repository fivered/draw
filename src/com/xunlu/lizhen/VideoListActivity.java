...
public class VideoListActivity extends DownloadActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		new ResultAsyncTask<ArrayList<VideoItem>>(this){
			...
			@Override
			protected void onPostExecuteSuc(ArrayList<VideoItem> result) {
				if(result!=null){
					videoItems=result;
					adapter.notifyDataSetChanged();
					noticeDownloadChange();
				}
				boolean isPlay = getIntent().getBooleanExtra("isPlay", false);
				if(isPlay){
					Log.d("fax", "Play!");
					new Handler().postDelayed(new Runnable() {
						...
						public void run() {
							...
							Button downBtn=(Button) view.findViewById(R.id.download_item_btn);
							currPosition = 0;
							isAutoPlay = true;
...
//		Log.d("fax", "onDownload,infos.size:"+infos.size());
